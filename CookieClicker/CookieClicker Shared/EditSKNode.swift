...
extension Group {
    static func bind(_ nodes: [SKNode], parent: SKNode? = nil) -> Group {
        let wow = Group()
        _ = nodes.map { $0.addTo(wow) }
        parent?.addChild(wow)
        return wow
    }
}
extension Label {
    var getTrueWidth: CGFloat {
        return Label(text: text!.split(separator: "\n")[0].s).width
    }
    static func text(_ yourString: String, color: UIColor = .black, font: FontNames = .hand, parent: SKNode? = nil) -> Label {
...
//        attrString.addAttributes([NSAttributedString.Key.foregroundColor : color, NSAttributedString.Key.font : UIFont.init(name: font.rawValue, size: 250)], range: range)
...
//        paragraphStyle.alignment = .center
        ...
        let wow = Label.init(text: yourString)
        wow.fontName = font.rawValue
        wow.fontColor = .black
...
extension Shape {
    func becomeButton(_ from: Hostable,_ to: Hostable.Type?) -> Self {
        touchBegan = { self.alpha = 0.5 }
        touchEnd = { self.alpha = 1 }
        touchEndedOn = { from.present(to ?? resolveScene()) }
        return self
    }
    func becomeExternalLink(_ to: Links) -> Self {
        touchBegan = { self.alpha = 0.5 }
